...
- 做新事情的时候，一定要想下 best practice 是什么，但不强求遵守，让用户选择
- 不用着急写代码，给方案。先把哲学问题讨论清楚。哲学问题就是这个东西的本质是什么，他应该负责什么。
- 用户如果说 push，就是把当前的 session 所有东西全部 commit，然后 push 到远端。具体分几个 commit 自己决定，不需要再问用户。
...
- Rust 模块组织 follow 2018+ 现代写法：优先 `foo.rs + foo/`。
